shinyServer(function(input, output, session) {
  ...
  data_init <- read_excel(filename,
    ...
    rename(
      log_levels = `SARS-CoV-2 Copies/L`,
    ...
    janitor::clean_names() %>%
    mutate(
      log_levels = ifelse(log_levels == "NA", NA, log_levels),
  ...
  data_rc_master <- reactive({
    data %>%
      ...
      mutate(
        label_both = paste0(station, ": ", scales::comma(log_levels, 1)),
        ...
        label_sampling_site = map(
          .x = label_sampling_site,
          .f = htmltools::HTML
        ),
      )
  })
  ...
  map_data <- reactive({
    ...
    if (!is.null(screen_width)) {
      ...
      if (screen_width <= 768) {
        set_view_fun <- function(data) {
          setView(data, -75.6, 39.65, 10)
        }
      } else {
        set_view_fun <- function(data) {
          setView(data, -75.55, 39.5, 10)
        }
      }
      ...
      data_prep %>%
        ...
        addPolygons(
          data = data_wilmington_wwtp$geometry, weight = 4, label = data_wilmington_wwtp$label_full, fillColor = "white", opacity = .7, color = "black", fillOpacity = 0, dashArray = "13", layerId = data_wilmington_wwtp$station,
        ...
        # ALL SHAPES OUTLINE
        addPolygons(
          data = data_prep$geometry, weight = 1, fillColor = col_pal(data_prep$log_levels), fill = T, opacity = .5, label = data_prep$label_full, fillOpacity = .5, color = "black", layerId = data_prep$station, group = "test",
          highlight = highlightOptions(
            weight = 1,
            stroke = TRUE,
            fillOpacity = .7,
        ...
        # NEWARK OUTLINE
        addPolygons(
          data = data_newark$geometry, weight = 2, label = data_newark$label_full, fillColor = "white", opacity = 1, color = "white", fillOpacity = 0, dashArray = "6", layerId = paste0(data_newark$station, "_layerid1"),
          highlight = highlightOptions(
            weight = 3,
            opacity = 1,
            stroke = TRUE,
        ...
        # WILMINGTON OUTLINE
        addPolygons(
          data = wilmington_combined_shape, weight = 1, label = HTML("<b>City of Wilmington</b><br>
        ...
        addCircleMarkers(
          data = data_christiana, ~longitude, ~latitude, label = ~label_sampling_site,
          opacity = .7, color = "white", fillColor = ~ col_pal(log_levels), fillOpacity = .8, weight = 2, radius = 7, layerId = ~station
        ) %>%
        # WILMINGTON MARKER
        addCircleMarkers(
          data = data_wilmington_wwtp, ~longitude, ~latitude, label = ~label_sampling_site,
  ...
  output$map2 <- renderLeaflet({
    ...
    if (!is.null(screen_width)) {
      ...
      county_outline_label <- paste0(
        "<b>New Castle County</b><br>
      ...
      leaflet(options = leafletOptions(zoomControl = FALSE, attributionControl = FALSE, worldCopyJump = TRUE)) %>%
        ...
        addPolygons(
          data = data_wilmington_wwtp$geometry, weight = 4, label = data_wilmington_wwtp$label_full, fillColor = "orange", opacity = .7, color = "black", fillOpacity = .4, dashArray = "13", layerId = "total_view",
          # highlight = highlightOptions(
          #     weight = 5,
          #     opacity = 1,
          #     stroke = TRUE,
  ...
  observeEvent(req(input$station_select),
    {
      ...
      if (is.na(st_bbox(filtered_data$geometry)[1])) {
        shape_geometry_centroid <- filtered_data %>%
  ...
  output$line_chart <- renderPlotly({
    data <- data_rc_master() %>%
      filter(station == req(input$station_select)) %>%
